...
(function ($) {
	...
	$.drawpad = function (element, options) {
		...
		const handleStopDraw = () => {
			...
			if(plugin.settings.onStrokeEnd && plugin.lastStrokePath.path){
				console.log("drawing stopped options")
				plugin.settings.onStrokeEnd(plugin.lastStrokePath)
			}
			plugin.lastStrokePath = {}
			plugin.lastStrokePath.path = []
		};
		const handleDraw = (event) => {
			...
			plugin.lastStrokePath.path.push({x: coordinate.x, y: coordinate.y});
			plugin.lastStrokePath.lineStyle = lineStyle;
			plugin.lastStrokePath.isEraser = (drawingType == "eraser")
		...
		plugin.addPath = (lastStrokePath) => {
			if(lastStrokePath && lastStrokePath.path && lastStrokePath.path.length > 0){
				const ctx = plugin.context;
				ctx.beginPath();
				ctx.globalCompositeOperation = "source-over";
				if(lastStrokePath.isEraser){
					ctx.globalCompositeOperation = 'destination-out'
				}
				
				ctx.lineWidth = lastStrokePath.lineStyle.width;
				ctx.strokeStyle = lastStrokePath.lineStyle.color;
				ctx.lineCap = lastStrokePath.lineStyle.type;
				ctx.moveTo(lastStrokePath.path[0].x, lastStrokePath.path[0].y);
				for(let i=0;i<lastStrokePath.path.length;i++){
					ctx.lineTo(lastStrokePath.path[i].x, lastStrokePath.path[i].y);
				}
				ctx.stroke();
				console.log("path added")
			}
			else{
				console.log("path not added :: length issue")
			}
